...
const displayWordDetails = (word) => {
  ...
  wordContainer.innerHTML = `  <div>
    ...
    word.pronunciation
        ...
        <h4 class="bangla-font mb-5">${
          word.meaning ? word.meaning : "শব্দ পাওয়া যাইনি"
        }</h4>
        <p class="font-bold mb-3">Example</p>
        <p class = 'mb-5'>${word.sentence}</p>
        <div class=" mb-4">
          <p class = 'mb-5'>synonyms</p>
  ...
  allWords.forEach((word) => {
    ...
    cardDiv.innerHTML = `
      <div class="text-center p-4 bg-white rounded-lg mb-5">
            <h2 class="font-bold mb-3 text-[20px]">${
              word.word ? word.word : "শব্দ পাওয়া যাইনি"
            }</h2> 
            <h4 class="font-semibold mb-3">Meaning/Pronounciation</h4>
            <h4 class="bangla-font">"${
              word.meaning ? word.meaning : "শব্দ পাওয়া যাইনি"
            } / ${
      word.pronunciation ? word.pronunciation : "pronounction পাওয়া যাইনি"
    }"</h4>
            <div class="flex justify-between items-center p-10">

              <button onclick="loadwordDetail(${
                word.id
              })" class="hover:bg-gray-500 p-3 rounded-md"> <i class="fa-solid fa-circle-info"></i></button>

              <button onclick="pronounceWord('${
                word.word
              }')" class="hover:bg-gray-500 p-3 rounded-md"><i class="fa-solid fa-volume-high"></i></button>
...
const serchBtn = document
  .getElementById("serch-btn")
  .addEventListener("click", () => {
    const inputBtn = document.getElementById("input-btn");

    const serchValue = inputBtn.value.trim().toLowerCase();
